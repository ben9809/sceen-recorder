...
std::vector<InputDeviceAudio> DeviceService::get_input_audio_devices() {
    ...
    AVInputFormat *fmt = av_find_input_format(DEVICE_ID_PULSE.c_str());
    AVDeviceInfoList *l;
...
PermissionsStatus DeviceService::check_permissions() {
    bool screenCaptureAllowed = true;
    bool microphoneCaptureAllowed = true;
    bool cameraCaptureAllowed = true;
