...
std::map<std::string, std::string> RecordingServiceImpl::get_device_options(
    ...
    if (deviceID == "x11grab") {
        int x = std::get<0>(config.getOutputResolution().value());
        int y = std::get<1>(config.getOutputResolution().value());
        std::string video_size = std::to_string(x) + "x" + std::to_string(y);
